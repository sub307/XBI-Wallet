...
/*** CoinBurn | DatBer 2018***/
#include "bannedTXIDs.h"
/****************************/
...
unsigned int nStakeMinAge2 = 4 * 60 * 60;
...
bool CheckTransactionBanned(const CTransaction& tx, bool fZerocoinActive, bool fRejectBadUTXO, CValidationState& state)
{
    ...
    if (tx.IsCoinBase()) {
        ...
        BOOST_FOREACH (const CTxIn& txin, tx.vin){
            ...
            /********************************/
            /**** Coin burn by DatBer 2018 ****/
            /*for(const map<string,uint32_t>::value_type& bad_tx : BANNED_TXIDs){
                if(txin.prevout.hash == uint256(bad_tx.first) && txin.prevout.n == bad_tx.second){
                    cout << "txid: "<< bad_tx.first << " index: "<< bad_tx.second << endl;
                    cout << "txout is Blocked because of Banned Transaction ID!" << endl;
                    cout << "******** Exit Scam? GET REKT! ********" << endl;
...
bool IsInitialBlockDownload()
{
    ...
    bool state = (chainActive.Height() < pindexBestHeader->nHeight - 24 * 6 ||
                  pindexBestHeader->GetBlockTime() < GetTime() - 6 * 60 * 60) && chainActive.Height() > 200; // ~144 blocks behind -> 2 x fork detection time
    if (!state)
        lockIBDState = true;
    return state;
}
